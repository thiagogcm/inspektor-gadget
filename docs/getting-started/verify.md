...
When deploying Inspektor Gadget using `kubectl gadget deploy`, the image will be automatically verified if the
`policy-controller` is already present in your kubernetes cluster.
