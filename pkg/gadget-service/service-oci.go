...
func (s *Service) GetOCIGadgetInfo(ctx context.Context, req *api.GetOCIGadgetInfoRequest) (*api.GetOCIGadgetInfoResponse, error) {
	...
	gadgetCtx := gadgetcontext.NewOCI(ctx, req.ImageName, s.runtime.ParamDescs().ToParams(), s.logger)

	gi, err := s.runtime.GetOCIGadgetInfo(gadgetCtx, req.ParamValues)
	if err != nil {
...
func (s *Service) RunOCIGadget(runGadget api.OCIGadgetManager_RunOCIGadgetServer) error {
	...
	gadgetCtx := gadgetcontext.NewOCI(runGadget.Context(), ociRequest.ImageName, runtimeParams, logger)
	...
	err = runtime.RunOCIGadget(gadgetCtx, ociRequest.ParamValues)
	if err != nil {
		return err
	}
	return nil
