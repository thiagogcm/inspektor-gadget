...
func (r *Runtime) GetOCIGadgetInfo(gadgetCtx runtime.GadgetContext, paramValueMap map[string]string) (*api.GadgetInfo, error) {
	...
	conn, err := r.getConnToRandomTarget(ctx, gadgetCtx.RuntimeParams())
	if err != nil {
...
func (r *Runtime) RunOCIGadget(gadgetCtx runtime.GadgetContext, paramMap map[string]string) error {
	gadgetCtx.Logger().Debugf("Params")
	for k, v := range paramMap {
		gadgetCtx.Logger().Debugf("- %s: %q", k, v)
	}

	// Install local operators after preparation is done (that is: after GadgetInfo has been loaded)
	gadgetCtx.OnPrepare(func() {
		...
		for _, op := range dataOperators {
			...
			opParams := opInst.ParamDescs(gadgetCtx).ToParams()
			opParams.CopyFromMap(paramMap, fmt.Sprintf("operator.%s.", opInst.Name()))

			err = opInst.Prepare(gadgetCtx, opParams)
			if err != nil {
