...
func (i *ebpfInstance) analyze() error {
	...
	for it.Next() {
		for _, entry := range prefixLookups {
			if typeName, ok := entry.prefixFunc(it.Type.TypeName()); ok {
...
func (i *ebpfInstance) register(gadgetCtx operators.GadgetContext) error {
	...
	for _, m := range i.tracers {
		...
		fields := make([]datasource.Field, 0, len(i.structs[m.StructName].Fields))
...
func (i *ebpfInstance) GadgetParams() []*api.Param {
	res := make([]*api.Param, 0, len(i.params))
