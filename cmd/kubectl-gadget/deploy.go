...
func runDeploy(cmd *cobra.Command, args []string) error {
	...
	if verifyImage {
		...
		if isPolicyControllerPresent {
			...
			// So, if users gives digest, we use it directly.
			// Otherwise, i.e. user gives a tag or the image itself, we extract the
			// repository from it and add "**" to glob it.
			var admissionImage string
			if strings.Contains(image, "@") {
				admissionImage = image
			} else {
				parts := strings.Split(image, ":")
				admissionImage = fmt.Sprintf("%s**", parts[0])
			}
