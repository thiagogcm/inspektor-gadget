...
func NewRunCommand(rootCmd *cobra.Command, runtime runtime.Runtime, hiddenColumnTags []string) *cobra.Command {
	...
	ociParams := ocihandler.OciHandler.ParamDescs().ToParams()
	...
	cmd := &cobra.Command{
		...
		PreRunE: func(cmd *cobra.Command, args []string) error {
			...
			ociParams.CopyToMap(paramValueMap, "oci.")

			// Fetch gadget information; TODO: this can potentially be cached
			info, err = runtime.GetOCIGadgetInfo(gadgetCtx, paramValueMap)
			if err != nil {
				return fmt.Errorf("fetching gadget information: %w", err)
			}

			for _, p := range info.Params {
				param := api.ParamToParamDesc(p).ToParam()
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			...
			gadgetCtx := gadgetcontext.NewOCI(ctx, args[0], runtimeParams, logger.DefaultLogger())
			...
			err := runtime.RunOCIGadget(gadgetCtx, paramValueMap)
			if err != nil {
				return err
			}
			return nil
